        ...
        if response.status == http_client.OK:
            ...
            if (
                (
                    ...
                    or any(e == "internal_failure" for e in (error_code, error_desc))
                )
                and retry < 1
                and should_retry
            ):
                retry += 1
                continue
            return response.status == http_client.OK, response.status, response_data
...
async def _token_endpoint_request(
    ...
    response_status_ok, response_status, response_data = await _token_endpoint_request_no_throw(
        request,
        token_uri,
        body,
    ...
    if not response_status_ok:
        client._handle_error_response(response_data, response_status)
